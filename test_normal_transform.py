...
def project_normal_to_screen(normal, center_point, camera_data_to_screen, image_shape):
    rotation_matrix = camera_data_to_screen['rotation_matrix']
    projection_matrix = camera_data_to_screen['projection_matrix']
    ...
    K_inv = np.linalg.inv(camera_data_to_screen["K"])
    center_point = np.dot(K_inv, np.append(center_point, 1)) #Camera space

    # Scale the normal for visibility and calculate its end point in world space
    #TODO transform normal to camera-space using projection
...
def select_points_and_draw_normal(video_filename, camera_data_to_world, camera_data_to_screen):
    ...
    cv2.circle(image, center_point_screen, 5, (0,0,255), -1)
...
with open(annotation_file, 'rb') as pb:
    ...
    for data in sequence.frame_annotations:
        ...
        camera_data_to_screen["K"] = np.array(data.camera.intrinsics).reshape(3,3)
